...
pub static WRITE_COMMANDS: [&str; 5] = [
    "BRSF:23", // minimum required for Cowin headphones to work
...
pub fn load_icons() -> Option<Vec<Icon>> {
    ...
    for icon in icon_paths {
        let (_, icon_rgba, icon_width, icon_height) = {
            let mut img =
                image::open(icon.as_ref().unwrap().path()).expect("Failed to open icon path");
            ...
            (
                icon.unwrap().file_name().into_string().unwrap(),
                rgba,
                width,
                height,
            )
